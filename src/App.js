...
class App extends Component {
  render() {
    return (
      <div className="App">
        <img alt="Herbular Curbular" className="Logo" src={logotype} />
        <UploadZone />
        <div className="Footer">
          <a href="#" target="_blank">
            built & designed by an herb
          </a>
        </div>
      </div>
    );
...
function applyMurderFilter(profile, banner) {
  ...
  ctx.drawImage(
    ...
    0,
    profile.width,
    profile.height, // source rectangle
    0,
    0,
    canvas.width,
    canvas.height,
  ...
  for (let i = 0; i < data.length; i += 4) {
    const saturation = 0.81 * data[i] + 0.11 * data[i + 1] + data[i + 2];
    data[i] = data[i];
    data[i + 1] = saturation;
    data[i + 2] = data[i + 2];
  }
  ...
  onDrop(files) {
    ...
    loadImage(files[0].preview, profile => {
      if (Math.abs(1 - profile.height / profile.width) > SQUARE_FORGIVENESS) {
        this.shake();
        this.setStatus(
          'the image has to be square-ish dimensions, sorry. try again?',
        );
        return;
      }
    ...
    return (
      <div className="Uploader">
        ...
        {!this.state.file ? null : (
          <a href={this.state.out} download="profile.png">
            <img
              alt="download"
              className="Download"
